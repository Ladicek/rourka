...
/**
 * Class for getting and parsing data from jenkins
 */
public class JenkinsDataProvider {
	...
	public List<Job> getJobs() throws IOException {
		...
		for (JsonValue jsonValue: jsonJobs){
			...
			job.setBuildLink(buildBuildUrl(job));
	...
	private void fillJobBuildInfo(Job job) throws IOException {
		...
		if (lastBuild.equals(JsonValue.NULL)){
			job.setLastBuildResult(new BuildResult(BuildStatus.UNKNOWN,0,null,false,null));
		}
		else {
	...
	/**
	 * Read json String value, which can possibly be NULL
	 * @param object Json object to find data in
	 * @param attribute Name of attribute that will be looked for
	 * @return Empty string if json value is null, string content otherwise
	 */
	...
	/**
	 * Read json Boolean Value, which can possibly be NULL
	 * @param object Json object to find data in
	 * @param attribute Name of the attribute that will be looked for
	 * @return False if json value is null, otherwise boolean content
	 */
	private Boolean readBooleanOrNull(JsonObject object, String attribute) {
		JsonValue jsonValue = object.get(attribute);
		return !jsonValue.getValueType().equals(JsonValue.ValueType.NULL) && object.getBoolean(attribute);
	}
}
